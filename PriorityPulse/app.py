    ...
    try:
        ...
        for row in data:
            try:
                ...
                rice = (reach * impact * (confidence/100)) / effort if effort > 0 else 0
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            required_columns = ['Feature', 'Reach', 'Impact', 'Confidence', 'Effort']
            ...
            return render_template('results.html', data=dict_data, columns=new_headers)
...
def download_file():
    ...
    return send_file(
        io.BytesIO(session['last_processed_data'].encode()),
        mimetype='text/csv',
        as_attachment=True,
...
def create_sample_csv():
    ...
    if not os.path.exists(sample_file):
        ...
        with open(sample_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Feature', 'Reach', 'Impact', 'Confidence', 'Effort'])
            for i in range(5):
                writer.writerow([features[i], reach[i], impact[i], confidence[i], effort[i]])
